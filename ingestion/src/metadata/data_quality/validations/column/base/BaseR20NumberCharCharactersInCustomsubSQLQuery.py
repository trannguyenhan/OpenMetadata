...
IN_SET_COUNT = "Not in set"


class BaseR20NumberCharCharactersInCustomsubSQLQueryValidator(BaseTestValidator):
    ...
    @abstractmethod
    def _get_length_of_value(self):
        raise NotImplementedError

    @abstractmethod
    def _run_results(self, column: Column, lengthOfValue: int, sql_expression: str):
        raise NotImplementedError
    ...
    def run_validation(self) -> TestCaseResult:
        ...
        length_of_value = self._get_length_of_value()

        try:
            sql_expression = cast(str, sql_expression)
            length_of_value = cast(int, length_of_value)
            column: Union[SQALikeColumn, Column] = self._get_column_name()
            res = self._run_results(column, length_of_value, sql_expression)
